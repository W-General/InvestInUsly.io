...
"-"+a))},this).compact().value()},destroy:function(a){if(a.isNew())return!1;this.localStorage().removeItem(this.name+"-"+a.id);this.records=d.reject(this.records,function(b){return b===a.id.toString()});this.save();return a},localStorage:function(){return localStorage},jsonData:function(a){return a&&JSON.parse(a)}});b.LocalStorage.sync=window.Store.sync=b.localSync=function(a,f,e){var c=f.localStorage||f.collection.localStorage,g,d,h=$.Deferred&&$.Deferred();try{switch(a){case "read":g=void 0!=f.id?
c.find(f):c.findAll();break;case "create":g=c.create(f);break;case "update":g=c.update(f);break;case "delete":g=c.destroy(f)}}catch(j){d=j.code===DOMException.QUOTA_EXCEEDED_ERR&&0===window.localStorage.length?"Private browsing is unsupported":j.message}g?(e&&e.success&&("0.9.10"===b.VERSION?e.success(f,g,e):e.success(g)),h&&h.resolve(g)):(d=d?d:"Record Not Found",e&&e.error&&("0.9.10"===b.VERSION?e.error(f,d,e):e.error(d)),h&&h.reject(d));e&&e.complete&&e.complete(g);return h&&h.promise()};b.ajaxSync=
b.sync;b.getSyncMethod=function(a){return a.localStorage||a.collection&&a.collection.localStorage?b.localSync:b.ajaxSync};b.sync=function(a,c,d){return b.getSyncMethod(c).apply(this,[a,c,d])};return b.LocalStorage});
